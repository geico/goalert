...
-- name: LabelFindAllByUserTarget :many
SELECT
    key,
    value
FROM
...
-- name: LabelDeleteKeyByUserTarget :exec
DELETE FROM labels
WHERE key = $1
    AND tgt_user_id = $2;
...
-- name: LabelSetByUserTarget :exec
INSERT INTO labels(key, value, tgt_user_id)
    VALUES ($1, $2, $3)
ON CONFLICT (key, tgt_user_id)
    DO UPDATE SET
        value = $2;
